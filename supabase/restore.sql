...
-- ═══════════════════════════════════════════
-- ДОПОЛНЕНИЯ: то, чего не хватало в schema.sql,
-- но что уже использует код (index.html)
-- ═══════════════════════════════════════════

-- trips.base_currency — loadSchedule читает trip.base_currency
ALTER TABLE trips ADD COLUMN base_currency text NOT NULL DEFAULT 'RUB';

-- профиль участника — saveProfile пишет phone/city/bio/dietary
ALTER TABLE members ADD COLUMN phone   text;
ALTER TABLE members ADD COLUMN city    text;
ALTER TABLE members ADD COLUMN bio     text;
ALTER TABLE members ADD COLUMN dietary text;

-- документы участников — файл хранится как data URL в file_data
...
CREATE INDEX idx_documents_trip ON documents(trip_id, created_at DESC);
