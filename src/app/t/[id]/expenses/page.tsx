...
function ExpensesInner() {
  ...
  const [tab, setTab] = useState<"ops" | "balance">(sp.get("tab") === "balance" ? "balance" : "ops");
    ...
    <>
      ...
      {tab === "balance" ? (
        ...
        <>
          ...
          <div className="fixed bottom-0 right-5 z-[45] flex flex-col items-center gap-1.5" style={{ bottom: "calc(var(--nav-h) + var(--safe-bottom) + 18px)" }}>
            <button type="button" aria-label="Добавить расход" onClick={() => setAddOpen(true)} className="flex h-14 w-14 items-center justify-center rounded-full bg-accent text-[28px] font-light text-white transition active:scale-95">+</button>
          </div>
        </>
      )}
...
function ExpenseRow({ e, onClick }: { e: Expense; onClick: () => void }) {
  ...
  const title = e.op_type === "transfer" ? `${t.name(e.paid_by)} → ${t.name(e.transfer_to ?? 0)}` : e.description || cat?.label || "Расход";
  ...
  return (
    <Card className={`flex cursor-pointer items-center gap-3.5 px-[18px] py-3.5 active:bg-surface-2 ${pending ? "opacity-60" : ""}`} onClick={onClick}>
      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-[18px] ${e.op_type === "income" ? "bg-good-soft" : "bg-bg"}`}>{icon}</div>
